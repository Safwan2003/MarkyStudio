...
const SceneRenderer = ({ scene, brand, themeStyles: globalThemeStyles }: { scene: VideoScene, brand: any, themeStyles: ThemeStyles }) => {
        ...
        switch (scene.id) {
            ...
            case 7:
                content = <PretaaSlot7Outro
                    ...
                    ctaUrl={scene.ctaUrl || scene.domain || "pretaa.com"}
                    screenshotUrl={scene.screenshotUrl}
            ...
            default:
                ...
                content = (
                    <GenericIntro
                        brandColor={brand?.accentColor}
                        bubbles={[scene.title || "Scene", scene.type || "Generic"]}
                    />
                );
        }
    }
    ...
    return (
        <AbsoluteFill>
            <DynamicCamera camera={scene.director?.layoutPlan?.camera} frame={frame}>
                <PerspectiveSwitcher perspective={scene.director?.layoutPlan?.compositionPerspective}>
                    {() => content}
                </PerspectiveSwitcher>
            </DynamicCamera>
...
export interface PretaaReactTemplateProps {
    plan: VideoPlan;
    [key: string]: unknown;
}

export type PretaaTemplateData = PretaaReactTemplateProps;

// Cinematic Global Overlay is now imported
...
export const PretaaReactTemplate: React.FC<PretaaReactTemplateProps> = ({ plan }) => {
    ...
    const effectivePlan: CommercialPlan = (plan && scenes.length > 0) ? plan : {
        brandName: 'Pretaa',
        brandColor: '#3b82f6',
    ...
    return (
        <AbsoluteFill style={{
            ...
            <AudioHandler scenes={effectiveScenes as any} audioTrack="https://res.cloudinary.com/ds5317tui/video/upload/v1734341907/Corporate_Upbeat_Background_Music_For_Videos_royalty_free_No_Copyright_Content_t4f9tp.mp3" />
                ...
                {effectiveScenes.map((scene, index) => (
                    <React.Fragment key={scene.id || index}>
                        <TransitionSeries.Sequence durationInFrames={Math.max(1, Math.floor((scene.duration || 5) * 30))}>
                            <SceneRenderer scene={scene as VideoScene} brand={effectiveBrand} themeStyles={themeStyles} />
